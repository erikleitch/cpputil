...
int print_obj(const NetObjTable *types, int id, void *obj)
{
 ...
 */
  if(!types) {
    DBPRINT(true, gcp::util::Debug::DEBUGANY, "print_obj: NULL types argument");
    return 1;
  };
/*
 ...
 */
  ...
  if(!info)
  {
    DBPRINT(true, gcp::util::Debug::DEBUGANY, "print_obj: no info for object id " << id);
    return 1;
  }
  
  DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, info->name);
/*
 * A NULL object is legal if the object contains no members.
 */
  if(!obj && info->nmember != 0) {
    DBPRINT(true, gcp::util::Debug::DEBUGANY, "print_obj: NULL object " << info->name);
    return 1;
  };
  
  if (info->nmember == 0)
  {
    DBPRINT(true, gcp::util::Debug::DEBUGANY, "print_obj: no members");
  }
 ...
 */
    ...
    case NET_ASCII:
      {
	DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "ASCII " << member->name << " " << (unsigned char*)data);
      };
      break;
    case NET_BYTE:
      DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "BYTE " << member->name << " " << ((unsigned char*)data)[0]);
      break;
    case NET_SHORT:
      DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "SHORT " << member->name << " " << *(unsigned short*)data);
      break;
    case NET_ENUM:
      if (*(unsigned short*)data < member->nenum)
      {
        DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "ENUM " << member->name << " " << member->etab[*(unsigned short*)data].name);
      } else
      {
	    DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, info->name << " " << member->name << " INVALID VALUE " << *(unsigned short*)data);
      }
      break;
    case NET_BOOL:
      DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "BOOL " << member->name << " " << *(unsigned short*)data);
      break;
    case NET_MASK:
      DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "MASK " << member->name << " " << *(unsigned int*)data);
      break;    
    case NET_INT:
      DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "INT " << member->name << " " << *(unsigned int*)data);
      break;
    case NET_FLOAT:
      DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "FLOAT " << member->name << " " << *(float*)data);
      break;
    case NET_DOUBLE:
      DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "DOUBLE " << member->name << " " << *(double*)data);
      break;
    default:
      DBPRINT_SIMPLE(true, gcp::util::Debug::DEBUGANY, "print_obj: Unknown data-type" << (int) member->type);
      return 1;
      break;
    };
  };
...
int obj_to_net(const NetObjTable *types, NetBuf *net, int id, void *obj)
{
  ...
  for(i=0; i<info->nmember; i++) {
    ...
    case NET_MASK:
    case NET_INT:
      if(net_put_int(net, member->ntype, (unsigned int* )data))
	return 1;
      break;
    case NET_FLOAT:
 ...
 */
    ...
    case NET_ASCII:
      {
	int len;
	if(net_get_int(net, 1, (unsigned int* )&len))
	  return 1;
	if(len >= member->ntype) {
	  lprintf(stderr,
    ...
    case NET_MASK:
    case NET_INT:
      if(net_get_int(net, member->ntype, (unsigned int* )data))
	return 1;
      break;
    case NET_FLOAT:
  ...
  //DBPRINT(true, gcp::util::Debug::DEBUGANY, std::endl << "RECEIVED ");    
  //print_obj(types, id, obj);
