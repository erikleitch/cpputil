...
#include "gcp/util/lprintf.h"
#include "gcp/util/print.h"
 ...
 */
typedef struct {
  LogStream id;             /* The stream id (LOG_STDOUT or LOG_STDERR) */
...
static void init_StreamState(LogStream id, StreamState *ss);
...
/*
 * If there are newline characters in buffer[] then we will need
...
/*
 * Keep a record of how many characters have been consumed from buffer[].
 */
    ndone += nnew;
/*
 ...
 */
  if((ss->nc > 0 || force) && ss->log_fn) {
    ...
    status = ss->log_fn(ss->buff, ss->id, ss->context);
  };
/*
 ...
 * to del_LogState().
 */
  init_StreamState(LOG_STDERR, &ls->err);
  init_StreamState(LOG_STDOUT, &ls->out);
  return ls;
}
