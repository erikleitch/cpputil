 ...
 */
    ...
    //std::cout << "About to call out_fn with " << n << " chars" << std::endl;
 ...
 * rendered as strings.
 */
	int min_len  = opts.min > 0 ? (int)ceil(log10((double)opts.min)) : 0;
	int prec_len = opts.prec >= 0 ?
	  (opts.prec==0 ? 1 : (int)ceil(log10((double)opts.prec))) : 0;
/*
