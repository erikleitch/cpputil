...
static int connect_to_server(char *host, int port, int protocol, int dowait, bool doThrow=false);
...
static int server_socket(int port, int protocol, int qlen)
{
  ...
  std::string proto_name;              /* The name of the protocol */
/*
 ...
 */
static int connect_to_server(char *host, int port, int protocol, int dowait, bool doThrow)
{
#ifndef VXW
 ...
 */
  if(connect(sock, (struct sockaddr *) &address, sizeof(address)) < 0) {
    if(dowait || errno != EINPROGRESS) {
      if(doThrow) {
	ThrowSimpleError(proto_name << " port " << port 
		   << " at address \"" << host 
		   << " \" is currently unreachable");
      } else {
	//	lprintf(stderr, 
	//		"%s port %d at address = \"%s\" is currently unreachable\n",
	//		proto_name, port, host);
      }
 ...
 *  return      int    The allocated socket, or -1 on error.
 */
int tcp_connect(char *host, int port, int dowait, bool doThrow)
{
