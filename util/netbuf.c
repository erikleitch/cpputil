 ...
 *
 * Input:
 *  size     int   The required buffer length in bytes.
 ...
 *  length    int    The buffer size in bytes. Note that this must
 ...
 *  nbytes    int     The number of bytes written to the buffer.
 * Output:
 *  return    int     The index of the next byte after the current
 *                     end of message, or -1 on error.
 */
int net_inc_nput(NetBuf *net, int nbytes)
{
/*
 ...
 *  size         int   The buffer length in bytes.
 * Output:
 *  return NetReadStr * The new stream input buffer, or NULL on error.
 */
NetReadStr *new_NetReadStr(int fd, int size)
{
  NetReadStr *nrs;
/*
 * Allocate the network buffer container.
...
/**.......................................................................
 * Constructor for use with gz file handle
 */
NetReadStr *new_NetReadStr(gzFile gzFp, int size)
{
  NetReadStr *nrs;
/*
 * Allocate the network buffer container.
 ...
 * Constructor for use with gz file handle
 */
NetReadStr *new_NetReadStr(gcp::util::FileStream* fs, int size)
{
  NetReadStr *nrs;
/*
 * Allocate the network buffer container.
...
void attach_NetReadStr(NetReadStr *nrs, int fd)
{
  if(nrs) {
    nrs->fd          = fd;
    nrs->gzFile_     = 0;
    nrs->readFn_     = readFnUncomp;
    nrs->readFnArgs_ = &nrs->fd;
    nrs->state = fd >= 0 ? NetReadStr::NET_READ_DONE : NetReadStr::NET_READ_CLOSED;
  };
}

void attach_NetReadStr(NetReadStr *nrs, gzFile gzFp)
{
  if(nrs) {
    nrs->fd          = -1;
    nrs->gzFile_     = gzFp;
    nrs->readFn_     = readFnGzip;
    nrs->readFnArgs_ = nrs->gzFile_;
    nrs->state = gzFp != 0 ? NetReadStr::NET_READ_DONE : NetReadStr::NET_READ_CLOSED;
  };
}

void attach_NetReadStr(NetReadStr *nrs, gcp::util::FileStream* fs)
{
  if(nrs) {
    nrs->fd          = -1;
    nrs->gzFile_     =  0;
    nrs->readFn_     =  readFnFileStream;
    nrs->readFnArgs_ =  fs;

    try {
      fs->checkStream();
      nrs->state = NetReadStr::NET_READ_DONE;
    } catch(...) {
      nrs->state = NetReadStr::NET_READ_CLOSED;
    }
 ...
 */
NetSendStr *new_NetSendStr(int fd, int size)
{
  NetSendStr *nss;
/*
 * Allocate the network buffer container.
 ...
 */
    errno = 0;
    nread = nrs->readFn_(nrs->readFnArgs_, net->buf + net->nput, nrs->msglen - net->nput);
 ...
 * substitute the real value of the target message size for
 * the previous dummy size.
 */
      if(net->nput >= nrs->msglen && nrs->state==NetReadStr::NET_READ_SIZE) {
	nrs->state = NetReadStr::NET_READ_DATA;
	if(net_get_int(net, 1, (unsigned int* )&nrs->msglen) ||
	   nrs->msglen < NET_PREFIX_LEN ||
	   // Martin used to check if the msglen was >= the net buffer
	   // size, but this doesn't make sense to me.  
	   // msglen == net->size is a perfectly valid condition
	   //
	   // nrs->msglen >= net->size) {
	   nrs->msglen > net->size) {
 ...
 */
      default:
	//	lprintf(stderr, "nrs_read_msg I/O error (%s)\n", strerror(errno));
	return nrs_state(nrs, NetReadStr::NET_READ_ERROR);
      };
    };
  };
 ...
 */
  {
    unsigned int ul_opcode;
    if(net_get_int(net, 1, &ul_opcode))
      return 1;
    *opcode = ul_opcode;
  };
  return 0;
...
{
  unsigned int ul_opcode;  /* The opcode in network form */
/*
 * Check arguments.
 */
  if(!net || opcode < 0) {
 ...
 */
  if(net_put_int(net, 1, (unsigned int* )&net->nput))
    return 1;
/*
 * Write the message-type code.
 */
  ul_opcode = opcode;
  return net_put_int(net, 1, &ul_opcode);
}
 ...
 *  ndata         int   The number of elements to be read.
 ...
 *                       1 - Error.
 */
int net_get_char(NetBuf *net, int ndata, unsigned char *data)
{
/*
 ...
 *                        1 - Error.
 */
int net_get_short(NetBuf *net, int ndata, unsigned short *data)
{
  u_short net_short;
  int i;
/*
 ...
 *  net         NetBuf * The network buffer to read from.
 *  ndata         int   The number of elements to be read.
 *  data unsigned int * A pointer to an output array of at least ndata
 *                       elements.
 * Output:
 *  return         int   0 - OK.
 *                       1 - Error.
 ...
 *                  1 - Error.
 */
int net_get_float(NetBuf *net, int ndata, float *data)
{
/*
 * Do we have sufficient data in the buffer to satisfy the request?
 */
 ...
 */
#else
  {
    int i;
    for(i=0; i<ndata; i++) {
      u_int net_int;  /* The float packed in a 32-bit network int */
...
/*
 * Extract the IEEE-754 float into a local 32-bit network int.
 */
      memcpy(&net_int, net->buf + net->nget, NET_FLOAT_SIZE);
 ...
 */
      sign_bit = net_int>>31U & 1U;
/*
 * Get the exponent (bits 31-24).
 */
      iexp = (net_int >> 23U & 0xffU) - 126U;
/*
 * Get the mantissa (bits 23-1).
 */
      mantissa = net_int & 0x7fffffU;
/*
 ...
 *                  1 - Error.
 */
int net_get_double(NetBuf *net, int ndata, double *data)
{
/*
 * Do we have sufficient data in the buffer to satisfy the request?
 */
 ...
 *                    1 - Error.
 */
int net_inc_nget(NetBuf *net, int nbytes)
{
/*
 * Do we have sufficient data in the buffer to satisfy the request?
 */
 ...
 *  ndata         int   The number of elements to be written.
 ...
 *                       1 - Error.
 */
int net_put_char(NetBuf *net, int ndata, unsigned char *data)
{
/*
 * Is there sufficient room in the network buffer?
 */
 ...
 *                        1 - Error.
 */
int net_put_short(NetBuf *net, int ndata, unsigned short *data)
{
  u_short net_short;
  int i;
/*
 ...
 *  net         NetBuf * The network buffer to write to.
 *  ndata         int   The number of elements to be written.
 *  data unsigned int * The array of 'ndata' elements to be copied into the
 *                       buffer.
 * Output:
 *  return         int   0 - OK.
 *                       1 - Error.
 */
int net_put_int(NetBuf *net, int ndata, unsigned int *data)
{
  u_int net_int;
  int i;
/*
 ...
 *                  1 - Error.
 */
int net_put_float(NetBuf *net, int ndata, float *data)
{
/*
 * Is there sufficient room in the network buffer?
 */
 ...
 */
  {
    int i;
    for(i=0; i<ndata; i++) {
      u_int net_int;       /* The output float packed into a network int */
      ...
      int sign_bit = host_float<0;       /* Sign bit (0 or 1) */
 ...
 */
      ...
      iexp += 126U;
/*
 * Compose the IEEE float in the 32-bit network int.
 */
      net_int = mantissa |  iexp << 23 | sign_bit << 31;
/*
 * Convert the host-byte-order int to network byte order
 * and copy it to the network buffer.
 */
      net_int = htonl(net_int);
      memcpy(net->buf + net->nput, &net_int, NET_FLOAT_SIZE);
 ...
 *                  1 - Error.
 */
int net_put_double(NetBuf *net, int ndata, double *data)
{
/*
 * Is there sufficient room in the network buffer?
 */
 ...
 * the host-byte-order double to the output int in host byte order.
 */
#if CPU==MC68040 || CPU==MC68060 || defined sparc
 ...
 */
  {
    ...
    for(i=0; i<ndata; i++) {
      ...
      u_int msbs, lsbs;                /* The most/least significant bytes */
 ...
 *
 * Form the 4 most and least significant bytes in two int's.
 */
      lsbs = modf(mantissa / two_pow_32, &mantissa) * two_pow_32;
      msbs = ((u_int)mantissa) | (iexp << 20U) | (sign_bit << 31U);
/*
 ...
 * Output:
 *  return   int    The requested size, or 0 if net==NULL.
 */
int size_NetBuf(NetBuf *net)
{
  if(!net) {
...
READ_FN(readFnGzip)
{
  gzFile gzFp = (gzFile)fp;
  ssize_t off = gzread(gzFp, buf, count);

  return off;
}

READ_FN(readFnFileStream)
{
  FileStream* fs = (gcp::util::FileStream*)fp;
  ssize_t off = fs->read(buf, count);
