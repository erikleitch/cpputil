...
void Port::writeBytes(Vector<unsigned char>& message)
{
  ...
  if(fd_ >= 0) {
    if(write(fd_, static_cast<unsigned char *>(&copy[0]), copy.size()) != copy.size()) {
      ThrowSysError("write()");
    }
  }
 ...
 * Write a message to the port
 */
void Port::writeBytes(Vector<unsigned char>& message, int fd)
{
  if(fd >= 0) 
    if(::write(fd, &message[0], message.size()-1) != message.size()-1)
      ThrowSysError("write()");
}
...
int Port::getNbyte(int fd) 
{
  ...
  if(ioctl((fd < 0 ? fd_ : fd), request, &nByte) != 0) {
    DBPRINT(true, Debug::DEBUGANY, "ioctl(FIONREAD) error. errno = " << errno);
    LogStream errStr;
